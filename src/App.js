...
const FII_FLOW = {
  ...
  FMCG:{flow:"Neutral",amt:"₹400Cr",wk:"→"},
  Auto:{flow:"Buying",amt:"₹1,200Cr",wk:"↑"},
  Energy:{flow:"Neutral",amt:"₹600Cr",wk:"→"},
  Power:{flow:"Buying",amt:"₹1,600Cr",wk:"↑"},
  Infrastructure:{flow:"Buying",amt:"₹2,200Cr",wk:"↑"},
  Metals:{flow:"Selling",amt:"₹-800Cr",wk:"↓"},
  Telecom:{flow:"Buying",amt:"₹1,400Cr",wk:"↑"},
  Consumer:{flow:"Neutral",amt:"₹200Cr",wk:"→"},
  Insurance:{flow:"Neutral",amt:"₹600Cr",wk:"→"},
...
export default function App() {
  ...
  const swingRows=useMemo(()=>rows.filter(r=>r.vr&&r.vr>1.3&&r.score>=48).sort((a,b)=>(b.vr||0)-(a.vr||0)),[rows]);
  ...
  return(
    <div style={{background:"#070b14",minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif",color:"#c8d8f0",fontSize:13}}>
      ...
      <div style={{background:"#0a0f1e",borderBottom:"1px solid #141e30",padding:"10px 16px",position:"sticky",top:0,zIndex:200}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <span style={{fontSize:18,fontWeight:900,letterSpacing:-0.8,background:"linear-gradient(135deg,#60a5fa,#34d399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>IndiaScope v4</span>
        ...
        {fiiPanelOpen&&<div style={{marginTop:10,background:"#070b14",borderRadius:10,padding:"10px 14px",border:"1px solid #141e30"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {Object.entries(FII_FLOW).map(([sec,data])=>(
              <div key={sec} style={{background:"#0a0f1e",borderRadius:6,padding:"5px 10px",border:`1px solid ${data.flow==="Buying"?"#34d39933":data.flow==="Selling"?"#f8717133":"#2a3a55"}`,minWidth:100}}>
                <div style={{fontSize:9,color:SCOL[sec]||"#60a5fa",fontWeight:600}}>{sec}</div>
                <div style={{fontSize:11,fontWeight:700,color:data.flow==="Buying"?"#34d399":data.flow==="Selling"?"#f87171":"#ffd740"}}>{data.wk} {data.flow}</div>
                <div style={{fontSize:9,color:"#2a3a55"}}>{data.amt}</div>
              </div>
            ))}
          </div>
        </div>}
      ...
      <div style={{display:"flex",gap:7,padding:"10px 16px",overflowX:"auto"}}>
        {[{l:"Strong Buy",v:rows.filter(r=>r.score>=78).length,c:"#00e676"},{l:"Buy",v:rows.filter(r=>r.score>=65&&r.score<78).length,c:"#69f0ae"},{l:"Hold",v:rows.filter(r=>r.score>=52&&r.score<65).length,c:"#ffd740"},{l:"Reduce/Sell",v:rows.filter(r=>r.score<52).length,c:"#ff5252"},{l:"⚡ Swing Alerts",v:swingRows.length,c:"#f59e0b"},{l:"Showing",v:rows.length,c:"#60a5fa"}].map(st=>(
              ...
              {mTab==="data"&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {[
                    ...
                    {l:"FII Sector Flow",v:FII_FLOW[sel.s]?.flow||"—",sub:FII_FLOW[sel.s]?.amt||"",ok:FII_FLOW[sel.s]?.flow==="Buying"},
                  ].map(it=>(
